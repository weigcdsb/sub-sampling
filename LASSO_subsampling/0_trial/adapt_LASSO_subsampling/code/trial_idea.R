...
## (d) uniform
unif.mse.adlasso <- rep(NA, length(r))
unif.mse.true <- rep(NA, length(r))
unif.mse.mle <- rep(NA, length(r))
for(i in 1:length(r)){
  ...
  for(j in 1:S){
    ...
    unif.y <- y[unif.idx]
    unif.X <- X[unif.idx, ]
    unif.beta.boot[j, ] <- adapt.lasso(unif.X, unif.y)
  }
  
  unif.mse.adlasso[i] <- mean(apply((unif.beta.boot -
                                      matrix(rep(beta.adpLasso, S),nrow = S, byrow = T))^2,
                                   1, sum))
  unif.mse.true[i] <- mean(apply((unif.beta.boot -
                                   matrix(rep(beta, S),nrow = S, byrow = T))^2,
                                1, sum))
  unif.mse.mle[i] <- mean(apply((unif.beta.boot -
                                  matrix(rep(beta.mle, S),nrow = S, byrow = T))^2,
                               1, sum))
}
...
lines(r, mVC.mse.adlasso, type = 'b', pch = '2',
      col = 2, lwd = 2)
lines(r, unif.mse.adlasso, type = 'b', pch = '3',
      col = 3, lwd =2)
abline(h = full.mse.adlasso, lty = 2, lwd = 2)
legend('topright', lwd = 2,
       col = c(1, 2, 3, 1), pch = c('1', '2', '3', NA),
       lty = c(1, 1, 1, 2),
       legend = c('mMSE', 'mVc', 'uniform', 'full'))
dev.off()
...
lines(r, mVC.mse.mle, type = 'b', pch = '2',
      col = 2, lwd = 2)
lines(r, unif.mse.mle, type = 'b', pch = '3',
      col = 3, lwd = 2)
...
lines(r, mVC.mse.true, type = 'b', pch = '2',
      col = 2, lwd = 2)
lines(r, unif.mse.true, type = 'b', pch = '3',
      col = 3, lwd = 2)
